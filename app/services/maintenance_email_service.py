...
async def send_maintenance_emails():
    print(f"[Scheduler] Ejecutando revisión de mantenimiento: {datetime.now()}")
    ...
    try:
        ...
        for row in reports:
            ...
            # Enviar correos
            await _send_client_email(fm, report)
            await _send_technician_email(fm, report)

            # Registrar notificación enviada
            _register_notification(cursor, conn, report)
...
def _register_notification(cursor, conn, report: dict):
...
async def _send_client_email(fm: FastMail, report: dict):
...
async def _send_technician_email(fm: FastMail, report: dict):
